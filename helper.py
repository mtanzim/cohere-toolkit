...
# # Update User
_ = requests.put(f"{base_url}/users/{user_id}", headers=headers, json={'fullname': 'new name'})

# Chat
response = requests.post('http://localhost:8000/v1/chat-stream', headers=headers, json={'message': 'who is bo burnham?', 'tools': [{'name': 'Wikipedia'}]})
...
## Agents
# Create Agent
response = requests.post(f"{base_url}/agents", headers=headers, json={'name': 'New agent', 'model': 'command-r', 'deployment': "Cohere Platform"})
...
# # Update Agent
_ = requests.put(f"{base_url}/agents/{agent_id}", headers=headers, json={"name": "new_name"})
...
# Update Conversation
_ = requests.put(f"{base_url}/conversations/{conversation_id}", headers=headers, json={"title": "new_title"})
