...
def get_tool_auth(db: Session, tool_id: str, user_id: str) -> ToolAuth:
    ...
    """
    return db.query(ToolAuth).filter(ToolAuth.tool_id == tool_id, ToolAuth.user_id == user_id).first()


# def update_tool_auth(db: Session, tool_auth: ToolAuth, new_tool_auth: UpdateToolAuth) -> ToolAuth:
#     """
#     Update an tool auth.

#     Args:
#       db (Session): Database session.
#       tool_auth (ToolAuth): ToolAuth to be updated.
#       new_tool_auth (UpdateToolAuth): New tool_auth.

#     Returns:
#       ToolAuth: Updated tool_auth.
#     """
#     for attr, value in new_tool_auth.model_dump(exclude_none=True).items():
...
def delete_tool_auth(db: Session, user_id: str, tool_id: str) -> None:
    ...
    Args:
        ...
        tool_id (str): Tool ID.
    """
    tool_auth = db.query(ToolAuth).filter(ToolAuth.tool_id == tool_id, ToolAuth.user_id == user_id)
    tool_auth.delete()
    db.commit()
